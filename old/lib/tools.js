...
function isNewProject(gorillaFile){
    ...
    if(isNew == 'yes'){
        ...
        variables.stateOfProject = 'new';

    }

    events.publish('PROMISEME', isNew);
...
function printLogo(){
    ...
    // Tengo que mostrar el logo en ASCII-ART
    figlet.text('gorilla js', {

        // font: '3D-ASCII',
        // font: 'Block',
        // font: 'Bigfig', // En minúsculas
        font: 'B1FF',
...
function createFolderTree(){
    ...
    fsx.ensureDirSync(path.join(variables.homeUserPathNodeJS, variables.proxyName, 'plugins'));
    fsx.ensureDirSync(path.join(variables.homeUserPathNodeJS, variables.proxyName, 'templates'));
...
function saveOldData(gorillaFile){
        ...
        if(data.hasOwnProperty('local')){
            ...
            if(data.local.hasOwnProperty('project')){
                ...
                if(data.local.project.hasOwnProperty('id')){
                    ...
                    data.oldData = data.local;
                    data.local = {
                        project: {
                            id: data.oldData.project.id
                        }
                    };
    ...
    variables.stateOfProject = 'force';

    events.publish('PROMISEME');

}

function fusionOldData(gorillaFile){
    ...
    if(fs.existsSync(gorillaFile)){
        ...
        if(data.hasOwnProperty('oldData')){
            ...
            merge = mergeJSON.merge(data.oldData, data.local);
            data.local = merge;
            delete data.oldData;
    ...
    if(configData.hasOwnProperty('messages')){
        ...
        if(configData.messages.hasOwnProperty('warnings')){
            ...
            if(configData.messages.warnings.hasOwnProperty(warning)){
                ...
                console.log('-'.bold.yellow, warning.bold.yellow);
...
function paramForced(step, name, value, save){
