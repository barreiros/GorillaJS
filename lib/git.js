...
module.exports = {
    ...
    lastCommitDate: function(branch){
        console.log(branch);

        var date;

        cross.execSync('git', ['show', branch, '-s', '--oneline', '--pretty=format:%ci'], function(err, stdout, stderr){
    ...
    listFiles: function(since, branch){
        events.publish('STEP', ['git-listfiles']);
        events.publish('VERBOSE', ['since ' + since + ' on branch ' + branch]);

        var output, files;

        output = {};

        cross.execSync('git', ['log', branch, '--name-only', '--oneline', '--no-commit-id', '--pretty=format:', '--diff-filter=AC', '--since="' + since + '"'], function(err, stdout, stderr){
            if (err) events.publish('ERROR', ['020']);
            events.publish('VERBOSE', [stderr]);
        ...
        cross.execSync('git', ['log', branch, '--name-only', '--oneline', '--no-commit-id', '--pretty=format:', '--diff-filter=MT', '--since="' + since + '"'], function(err, stdout, stderr){
            if (err) events.publish('ERROR', ['020']);
            events.publish('VERBOSE', [stderr]);
        ...
        cross.execSync('git', ['log', branch, '--name-only', '--oneline', '--no-commit-id', '--pretty=format:', '--diff-filter=D', '--since="' + since + '"'], function(err, stdout, stderr){
            if (err) events.publish('ERROR', ['020']);
            events.publish('VERBOSE', [stderr]);

            var dateAdded, dateDeleted;

            files = stdout.toString().split('\n');
            files = files.filter(function(value){
                if(value.length !== ''){
                    cross.execSync('git', ['log', branch, '--oneline', '--pretty=format:%ct', '--since="' + since + '"', '--diff-filter=AC', '--', value], function(err, stdout, stderr){
                        dateAdded = stdout;
                    });
                    cross.execSync('git', ['log', branch, '--oneline', '--pretty=format:%ct', '--since="' + since + '"', '--diff-filter=D', '--', value], function(err, stdout, stderr){
                        dateDeleted = stdout;
                    });

                    if(dateAdded < dateDeleted){
                        return value;
                    }
                }
            });
