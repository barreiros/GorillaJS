...
module.exports = {
    ...
    listFiles: function(since, branch){
        ...
        cross.spawnSync('git', ['log', branch, '--name-only', '--oneline', '--no-commit-id', '--pretty=format:', '--diff-filter=AC', '--since="' + since + '"'], function(err, stdout, stderr){
            if (err) events.publish('ERROR', ['020']);
            events.publish('VERBOSE', [stderr]);
        ...
        cross.spawnSync('git', ['log', branch, '--name-only', '--oneline', '--no-commit-id', '--pretty=format:', '--diff-filter=MT', '--since="' + since + '"'], function(err, stdout, stderr){
            if (err) events.publish('ERROR', ['020']);
            events.publish('VERBOSE', [stderr]);
        ...
        cross.spawnSync('git', ['log', branch, '--name-only', '--oneline', '--no-commit-id', '--pretty=format:', '--diff-filter=D', '--since="' + since + '"'], function(err, stdout, stderr){
            if (err) events.publish('ERROR', ['020']);
            events.publish('VERBOSE', [stderr]);
            ...
            files = files.filter(function(value){
                if(value.length !== ''){
                    dateAdded = cross.spawnSync('git', ['log', branch, '--oneline', '--pretty=format:%ct', '--since="' + since + '"', '--diff-filter=AC', '--', value]);
                    dateDeleted = cross.spawnSync('git', ['log', branch, '--oneline', '--pretty=format:%ct', '--since="' + since + '"', '--diff-filter=D', '--', value]);

                    if(dateAdded.out < dateDeleted.out){
                        return value;
                    }
                }
            });
