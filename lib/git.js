...
module.exports = {
    ...
    listFilesChanged: function(){
        events.publish('STEP', ['git-fileschanged']);

        // git show --name-only --oneline --no-commit-id --pretty="format:"
        // Tengo que añadir la ruta desde el raíz para que sean rutas absolutas.
        var files;

        cross.execSync('git', ['show', '--name-only', '--oneline', '--no-commit-id', '--pretty=format:'], function(err, stdout, stderr){
    ...
    listFilesRemoved: function(){
        events.publish('STEP', ['git-filesdeleted']);

        var files;

        cross.execSync('git', ['show', '--name-only', '--oneline', '--no-commit-id', '--pretty=format:', '--diff-filter=MD'], function(err, stdout, stderr){
            if (err) events.publish('ERROR', ['020']);
            events.publish('VERBOSE', [stderr]);
