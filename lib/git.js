...
module.exports = {
    ...
    listCommits: function(branch){

        var commits;

        cross.spawnSync('git', ['log', branch, '--oneline', '--pretty=format:%H'], function(err, stdout, stderr){
        ...
        cross.spawnSync('git', ['log', branch, '--name-only', '--oneline', '--no-commit-id', '--pretty=format:', '--diff-filter=D', '--since', '"' + since + '"', '--until', '"' + until + '"'], function(err, stdout, stderr){
            ...
            files = files.filter(function(value){
                if(value.length !== ''){
                    ...
                    if(backward){
                        if(dateDeleted[dateDeleted.length - 1] < dateAdded[dateAdded.length - 1]){
