...
function login(name, password){
    ...
    if(name && password){
        ...
        credentials = JSON.parse(fs.readFileSync(path.join(variables.homeUserPath, variables.proxyName, 'config.json')));
...
function requestToken(name, password){
    ...
    request.post(options, function(error, response, body){
        ...
        if(body.hasOwnProperty('token')){ // Si es correcto guardo las credenciales y devuelvo un token para esta operación.
            ...
            config = JSON.parse(fs.readFileSync(path.join(variables.homeUserPath, variables.proxyName, 'config.json')));
            config['name'] = name;
            config['password'] = password;
            fs.writeFileSync(path.join(variables.homeUserPath, variables.proxyName, 'config.json'), JSON.stringify(config, null, '\t'));

            tools.paramForced('login', 'token', body.token, false);

            events.publish('STEP', ['Login OK']);
