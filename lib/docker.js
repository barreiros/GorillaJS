...
module.exports = {
    ...
    start: function(machine, composeFile, domain){
        events.publish('STEP', ['docker-start']);

        if(platform !== 'linux'){
            cross.exec('eval $(docker-machine env ' + machine + ') && docker-compose -f ' + composeFile + ' -p ' + domain + ' up --force-recreate -d', function(err, stdout, stderr){
            ...
            cross.exec('docker-compose -f ' + composeFile + ' -p ' + domain + ' up --force-recreate -d', function(err, stdout, stderr){
                if (err) events.publish('ERROR', ['007']);
                events.publish('VERBOSE', [stderr]);
