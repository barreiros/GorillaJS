...
module.exports = {
    ...
    gorigit: function(path){
        ...
        cross.exec('docker ps -q --filter="name=gorigit"', function(err, stdout, stderr){

            if(stdout === ''){

                console.log(path);

                cross.exec('docker rm gorigit && docker run -d --name gorigit -v ' + path + ':/var/gorillajs gorillajs/apache-base', function(err, stdout, stderr){
    ...
    templates: function(template, repo){

        cross.exec('docker exec gorigit git clone ' + repo + ' /var/gorillajs/templates', function(err, stdout, stderr){
