...
module.exports = {
    ...
    loggingBase: function(composeFile, slug){

        cross.exec('docker ps -q --filter="name=' + slug + '"', function(err, stdout, stderr){
    ...
    logging: function(composeFile, slug, loggingPath, templatePath){
        ...
        cross.exec('docker-compose -f ' + composeFile + ' -p ' + slug + ' ps -q', function(err, stdout, stderr){
            ...
            cross.exec('docker inspect --format="{{.Name}}" ' + stdout.replace('\n', ' '), function(err, stdout, stderr){
                ...
                names = getContainersName(stdout, true);
    ...
    base: function(composeFile, slug){
...
function getContainersName(list, includeProxy){

    var containers;

    containers = list.split('\n').filter(function(result){

        if(result !== '' && (result !== '/gorillajsproxy' || includeProxy)){
