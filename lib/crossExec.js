...
module.exports = {
    ...
    removeFiles: function(files, remote, exclude){
                    ...
                    if(typeof files === 'object'){
                        ...
                        if(files.length > 0){
                            ...
                            files.forEach(function(file, index){
                                sftp.unlink('/var/gorilla/gorilladavid/application/' + file, function(err){
                                    if (err) events.publish('WARNING', ['012']);
                                    events.publish('VERBOSE', [err]);

                                    sftp.readdir(path.dirname('/var/gorilla/gorilladavid/application/' + file), function(err, list){
                                        events.publish('VERBOSE', [err]);

                                        if(!list){
                                            sftp.rmdir(path.dirname('/var/gorilla/gorilladavid/application/' + file), function(err){
