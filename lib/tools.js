...
module.exports = function (env){
    ...
    module.param = function(step, name, options){
        ...
        var value, stats, exist, key, save;
        ...
        if(exist){
            ...
            if(options){
                key = prompt.keyInSelect(options, '[GorillaJS]'.bold.grey + '[Value]'.bold.green + ' Select from below the ' + step + ' ' + name + ' value: ', {cancel: false, guide: false});
                if(options[key].toLowerCase() === 'other'){
                    value = prompt.question('[GorillaJS]'.bold.grey + '[Value]'.bold.green + ' Enter the ' + step + ' ' + name + ' value: ', {});
                }else{
                    value = options[key];
                }
            }else{
                if(name.indexOf('pass') > -1 || name.indexOf('token') > -1){
                    value = prompt.question('[GorillaJS]'.bold.grey + '[Value]'.bold.green + ' Enter the ' + step + ' ' + name + ' value (GorillaJS not store your passwords): ', {hideEchoBack: true, mask: ''});
                    console.log(value);
                    save = false;
                }else{
                    value = prompt.question('[GorillaJS]'.bold.grey + '[Value]'.bold.green + ' Enter the ' + step + ' ' + name + ' value: ', {});
