...
function createBaseEnvironment(projectPath, templatesPath, gorillaPath, homePath, gorilla, gorillaFolder, messagesFile, homeUserPath){
    ...
    if(projectPath === gorillaPath){
        ...
        gorillaFileGlobal = path.join(homePath, gorilla);
        ...
        if(!fs.existsSync(gorillaFileGlobal)){

            fsx.ensureFileSync(gorillaFileGlobal);
            config[env]['global'] = {};

        }else{

            configGlobal = JSON.parse(fs.readFileSync(gorillaFileGlobal));
            if(configGlobal.hasOwnProperty('global')){

                config[env]['global'] = configGlobal['global'];
...
function writeConfigFile(){
    ...
    globalData = {
        global: Object.assign({}, config[env]['global'])
    };

    fsx.ensureFileSync(gorillaFile);
    fsx.ensureFileSync(gorillaFileGlobal);

    fs.writeFileSync(gorillaFileGlobal, JSON.stringify(globalData, null, '\t'));
    delete config[env]['global'];
    fs.writeFileSync(gorillaFile, JSON.stringify(config, null, '\t'));
    config[env]['global'] = globalData['global'];
