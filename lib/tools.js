...
function configuration(environment, force){
    env = environment;
    forcePrompts = force;
...
function addLine(file, line, sudo){
...
function param(step, name, options, filter, save){
    ...
    if(exist && !forcePrompts){
        ...
        if(forcePrompts){
            if(config[env].hasOwnProperty(step)){
                if(config[env][step].hasOwnProperty(name)){
                    defaultValue = config[env][step][name];
                }
            }
        }
                ...
                inquirer.prompt([{
                    ...
                    if(answers.option.toLowerCase() === 'other'){
                        inquirer.prompt([{
                            ...
                            events.publish('PROMISEME', answers.option);
                        });
                    }else{
                        if (save) saveValue(step, name, answers.option, filter);
                        events.publish('PROMISEME', answers.option);
...
function createBaseEnvironment(projectPath, templatesPath, gorillaPath, gorilla, gorillaFolder, messagesFile){
...
function createTemplateEnvironment(projectPath, gorillaFolder, gorilla, messagesFile, templateName){
    events.publish('STEP', ['tools-createtemplate']);

    var messagesFileTemplate, gorillaFileDocker, configDocker, messagesTemplate, port;

    messagesFileTemplate = projectPath + '/' + gorillaFolder + '/' + templateName + '/' + messagesFile;
    ...
    gorillaFileDocker = projectPath + '/' + gorillaFolder + '/' + templateName + '/' + gorilla;
