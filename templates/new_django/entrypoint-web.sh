...
pip install -r /var/www/{{project.domain}}/requirements.txt &&
...
if [ ! -e /var/www/{{project.domain}}/manage.py ]; then
    ...
    django-admin.py startproject /var/www/{{project.domain}}/{{project.slug}} . &&
...
if [ ! -d "static" ]; then
    ...
    python /var/www/{{project.domain}}/manage.py collectstatic --noinput
